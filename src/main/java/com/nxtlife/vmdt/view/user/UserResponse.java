package com.nxtlife.vmdt.view.user;
...
public class UserResponse implements Response {
	...
	@Schema(description = "User's mobile No",example = "9898989898")
	...
	public UserResponse(Long id, String name, Boolean active, String username, String email,String mobileNo, String contactNo,
			String picUrl) {
		super();
		this.id = id;
		this.active = active;
	...
	public static UserResponse get(User user) {
		if (user != null) {
			UserResponse response = new UserResponse(user.getId() == null ? user.getUserId() : user.getId(),
					user.getName(), user.getActive(), user.getUsername(), user.getEmail(), user.getMobileNo(),user.getContactNo(),
					user.getPicUrl());

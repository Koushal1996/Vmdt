...
import com.nxtlife.vmdt.dao.jpa.*;
import com.nxtlife.vmdt.entity.common.RoleAuthorityKey;
import com.nxtlife.vmdt.entity.common.UserRoleKey;
import com.nxtlife.vmdt.entity.user.*;
import com.nxtlife.vmdt.ex.NotFoundException;
import com.nxtlife.vmdt.ex.ValidationException;
import com.nxtlife.vmdt.service.BaseService;
import com.nxtlife.vmdt.service.UserService;
import com.nxtlife.vmdt.util.AuthorityUtils;
...
public class UserServiceImpl extends BaseService implements UserDetailsService, UserService {
    ...
    public List<UserResponse> findAll() {
        ...
        userJpaDao.findAll().stream()
                .forEach(user -> userResponseList.add(new UserResponse(user.getId(), user.getName(),
                        user.getActive(), user.getUsername(), user.getEmail(), user.getMobileNo(), user.getContactNo(), user.getPicUrl())));
        List<Long> roleIds = roleDao.getAllIdsByActive(true);
